...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="text-dark">📦 Data Mutasi Barang</h1>
        <div>
            <a href="{{ route('mutations.create') }}" class="btn btn-primary shadow-sm">
                <i class="fas fa-plus me-1"></i> Input Mutasi
            </a>
        </div>
    </div>
    ...
    <form method="GET" action="{{ route('mutations.index') }}" class="row mb-4" id="filterForm">
        <div class="col-12">
            <div class="card shadow-sm">
                ...
                <div class="card-footer d-flex justify-content-end gap-2">
                    ...
                    <a href="{{ route('mutations.index') }}" class="btn btn-outline-secondary shadow"><i class="fas fa-redo"></i> Reset</a>
                </div>
            </div>
        </div>
    </form>
    
    <form id="bulkDeleteForm" method="POST" action="{{ route('mutations.bulkDestroy') }}" style="display:none;">@csrf<div id="bulkDeleteIdsContainer"></div></form>

    <div class="d-flex mb-3 gap-2">
        <a href="{{ route('mutations.index', array_merge(request()->query(), ['mode' => 'resume'])) }}" class="btn {{ $mode == 'resume' ? 'btn-info text-white shadow-lg' : 'btn-outline-info' }}">Resume (Net Stock)</a>
        <a href="{{ route('mutations.index', array_merge(request()->query(), ['mode' => 'details'])) }}" class="btn {{ $mode == 'details' ? 'btn-info text-white shadow-lg' : 'btn-outline-info' }}">Details (Records)</a>
    </div>

    <div class="card shadow-lg">
        <div class="card-header bg-info text-black">{{ $mode == 'details' ? 'Hasil Data - Details' : 'Hasil Data - Net Stock (Mutasi - Penjualan)' }}</div>
                    ...
                    @if ($mode == 'details')
                        <table class="table table-bordered table-striped table-hover table-sm mb-0">
                            <thead class="bg-light sticky-top">
                                <tr>
                                    <th><input type="checkbox" id="select-all-details"></th><th class="text-center">Aksi</th><th>Tanggal</th><th>Material</th><th>Part</th><th>Lot</th><th>Tipe</th><th class="text-end">Berat (KG)</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($items as $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->material }}</td><td>{{ $item->part }}</td><td>{{ $item->no_lot }}</td>
                                        <td>
    ...
    <div class="modal fade" id="pivotDetailModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-xl"> 
            <div class="modal-content">
                <div class="modal-header bg-info text-white"><h5 class="modal-title">Detail Mutasi</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
