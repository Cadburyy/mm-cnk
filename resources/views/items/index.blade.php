...
<div class="container-fluid">
                        ...
                        <div class="row g-3">
                         ...
                         <div class="row g-3 mt-4">
                            <div class="col-6">
                                ...
                                <input list="materials" name="material_term" class="form-control" value="{{ $material_term }}" oninput="this.value = this.value.toUpperCase()">
                                <datalist id="materials">@foreach($materials as $mat)<option value="{{ $mat }}">@endforeach</datalist>
                            </div>
                            <div class="col-6">
                                <label class="form-label">Part</label>
                                <input list="parts" name="part_term" class="form-control" value="{{ $part_term }}" oninput="this.value = this.value.toUpperCase()">
                                <datalist id="parts">@foreach($parts as $part)<option value="{{ $part }}">@endforeach</datalist>
                            </div>
                        </div>
                    @endif
                </div>
    ...
    <div class="d-flex mb-3 gap-2">
        <a href="{{ route('items.index', array_merge(request()->query(), ['mode' => 'resume'])) }}" class="btn {{ $mode == 'resume' ? 'btn-info text-white shadow-lg' : 'btn-outline-info' }}">Resume (Monthly Pivot)</a>
        <a href="{{ route('items.index', array_merge(request()->query(), ['mode' => 'details'])) }}" class="btn {{ $mode == 'details' ? 'btn-info text-white shadow-lg' : 'btn-outline-info' }}">Details (All Records)</a>
    </div>

    <div class="card shadow-lg">
        <div class="card-header bg-info text-black">
            {{ $mode == 'details' ? 'Hasil Data - Details' : 'Hasil Data - Resume' }}
        </div>
                            ...
                            <tbody>
                                @foreach($summary_tree as $material => $matData)
                                    ...
                                    <tr class="resume-row parent-row" style="background-color: #f0f0f0; cursor: pointer;" 
                                        ...
                                        @if (count($months) > 0)
                                            @foreach($months as $m)
                                                <td class="text-end font-monospace">{{ number_format($matData['months_all'][$m['key']] ?? 0, 0, ',', '.') }}</td>
                                            @endforeach
                                        @endif
                                        <td class="text-end fw-bold font-monospace bg-light">{{ number_format($matData['total_all'], 0, ',', '.') }}</td>
                                    </tr>
                                    ...
                                    @foreach($matData['parts'] as $part => $partData)
                                        ...
                                        <tr class="child-mat-{{ $matUniqueId }} parent-row" style="display:none; background-color: #fff; cursor:pointer;"
                                            ...
                                            <td class="ps-4 fw-bold text-dark">
                                                <i class="fas fa-cube me-1 text-muted"></i> {{ $part }} <span class="badge bg-info ms-1" style="font-size:0.6em">Part Total</span>
                                            </td>
                                            @if (count($months) > 0)
                                                @foreach($months as $m) <td class="text-end font-monospace small">{{ number_format($partData['months_all'][$m['key']] ?? 0, 0, ',', '.') }}</td> @endforeach
                                            @endif
                                            <td class="text-end font-monospace fw-bold small">{{ number_format($partData['total_all'], 0, ',', '.') }}</td>
                                        </tr>
                                        
                                        <tr class="child-part-{{ $partUniqueId }} parent-row" style="display:none; background-color: #fdfdfd; cursor:pointer;"
                                            data-id-list="{{ $partIds }}" data-metric="gkg" data-level="metric">
                                            <td></td>
                                            <td class="ps-5 text-dark small">
                                                <i class="fas fa-check-circle me-1"></i> Produk (Barang Jadi)
                                            </td>
                                            @if (count($months) > 0)
                                                @foreach($months as $m) <td class="text-end font-monospace small text-muted">{{ number_format($partData['months_gkg'][$m['key']] ?? 0, 0, ',', '.') }}</td> @endforeach
                                            @endif
                                            <td class="text-end font-monospace small">{{ number_format($partData['total_gkg'], 0, ',', '.') }}</td>
                                        </tr>

                                        <tr class="child-part-{{ $partUniqueId }} parent-row" style="display:none; background-color: #fdfdfd; cursor:pointer;"
                                            data-id-list="{{ $partIds }}" data-metric="scrap" data-level="metric">
                                            <td></td>
                                            <td class="ps-5 text-dark small">
                                                <i class="fas fa-trash-alt me-1"></i> Scrap
                                            </td>
                                            @if (count($months) > 0)
                                                @foreach($months as $m) <td class="text-end font-monospace small text-muted">{{ number_format($partData['months_scrap'][$m['key']] ?? 0, 0, ',', '.') }}</td> @endforeach
                                            @endif
                                            <td class="text-end font-monospace small">{{ number_format($partData['total_scrap'], 0, ',', '.') }}</td>
                                        </tr>

                                        <tr class="child-part-{{ $partUniqueId }} parent-row" style="display:none; background-color: #fdfdfd; cursor:pointer;"
                                            data-id-list="{{ $partIds }}" data-metric="cakalan" data-level="metric">
                                            <td></td>
                                            <td class="ps-5 text-dark small">
                                                <i class="fas fa-recycle me-1"></i> Cakalan
                                            </td>
                                            @if (count($months) > 0)
                                                @foreach($months as $m) <td class="text-end font-monospace small text-muted">{{ number_format($partData['months_cakalan'][$m['key']] ?? 0, 0, ',', '.') }}</td> @endforeach
                                            @endif
                                            <td class="text-end font-monospace small">{{ number_format($partData['total_cakalan'], 0, ',', '.') }}</td>
                                        </tr>
...
$(function() {
    ...
    $(document).on('click', '.toggle-btn', function(e) {
        ...
        if (target.is(':visible')) { target.hide(); icon.removeClass('fa-minus').addClass('fa-plus'); }
        else { target.show(); icon.removeClass('fa-plus').addClass('fa-minus'); }
    });
    ...
    $(document).on('click', '.parent-row', function(e) {
        ...
        $('#detail-content').hide(); $('#detail-loading').show(); $('#monthly-subtotals-list').empty();
        $('#monthly-subtotals-container').hide();
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                headerTitle = headerTitle.replace(' [Total Berat (GKG + Scrap + Cakalan)]', '');
                ...
                $('#detail-content .mb-3:first').html(`
                    <div class="card bg-light border-0">
                        <div class="card-body py-2">
                            <span class="text-muted small text-uppercase fw-bold">${topLabel}</span>
                            <div class="fw-bold text-primary fs-5">${headerTitle}</div>
                        </div>
                    </div>
                `);
                
                $('#modal-metric-total').text(formatNumberJS(res.total_display) + ' Kg');
                ...
                if (pivotSelections.length > 0 && res.monthly_subtotals && Object.keys(res.monthly_subtotals).length > 0) {
                    ...
                    for (const ym in res.monthly_subtotals) {
                        subtotalsHtml += `<div class="p-2 border rounded shadow-sm bg-light"><div class="small text-muted fw-bold">${formatMonthYear(ym)}</div><div class="fw-bold font-monospace text-dark">${formatNumberJS(res.monthly_subtotals[ym])}</div></div>`;
                    }
                    $('#monthly-subtotals-list').html(subtotalsHtml);
                    $('#monthly-subtotals-container').show();
                } else {
                     $('#monthly-subtotals-container').hide();
                }
                ...
                if(res.details && res.details.length > 0) {
                    let tableHead = '<tr><th>Tanggal</th><th>Mat</th><th>Part</th><th>Lot</th><th>Barang Jadi (KG)</th>';
                    if (level !== 'material') {
                        tableHead += '<th>Scrap (KG)</th><th>Cakalan (KG)</th>';
                    }
                    tableHead += '</tr>';
                    ...
                    res.details.forEach(d => {
                        html += `<tr><td>${formatDateJS(d.tanggal)}</td><td>${d.material}</td><td>${d.part||'-'}</td><td>${d.no_lot||'-'}</td><td class="text-end fw-bold">${formatNumberJS(d.gkg)}</td>`;
                        if (level !== 'material') {
                            html += `<td class="text-end fw-bold">${formatNumberJS(d.scrap)}</td><td class="text-end fw-bold">${formatNumberJS(d.cakalan)}</td>`;
                        }
                        html += `</tr>`;
                    });
                    html += '</tbody></table>';
